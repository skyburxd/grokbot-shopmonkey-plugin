...
const tools = [
  ...
  },
  {
    ...
    description:
      "POST /v3/appointment — create an appointment. Required: name, startDate, endDate, color (aqua|black|blue|brown|gray|green|orange|purple|red|yellow). Optional: customerId, vehicleId, orderId, note, useEmail, useSMS, sendConfirmation, sendReminder, technicianIds, allDay. When sendConfirmation / sendReminder are omitted, both default to true. Returns official API JSON.",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Appointment name (e.g. Oil change)." },
        startDate: { type: "string", description: "Start date and time of the appointment." },
        endDate: { type: "string", description: "End date and time of the appointment." },
        color: {
          type: "string",
          enum: [
            "aqua",
        ...
        sendConfirmation: {
          type: "boolean",
          description:
            "Send confirmation at save time. Defaults to true when omitted.",
        },
        sendReminder: {
          type: "boolean",
          description:
            "Send reminder before the appointment. Defaults to true when omitted.",
        },
        technicianIds: {
          type: "array",
          items: { type: "string" },
          description: "Technician ids to assign.",
        },
        allDay: { type: "boolean" },
      },
...
async function callTool(name, args) {
  ...
  switch (name) {
    ...
    case "shopmonkey_create_appointment": {
      const errName = need(a, "name");
      if (errName) return errName;
      const errStart = need(a, "startDate");
      if (errStart) return errStart;
      const errEnd = need(a, "endDate");
      if (errEnd) return errEnd;
      const errColor = need(a, "color");
      if (errColor) return errColor;
      const body = pick(a, [
        "name",
        "startDate",
        "endDate",
